 ...
 */
public class LineXGenerator2 implements Task {
	...
	public LineXGenerator2(float centerX, float centerY, float size, int tr) {
	...
	public LineXGenerator2 generateRect(int num) {
	...
	public LineXGenerator2 generateCircle(int num) {
		FXPolygon[] array = new FXPolygon[num];
		disX = new float[num];
		for(int i = 0; i < num; i++) {
			array[i] = new FXCircle(posX, centerY, 20, (float) (Math.PI * 6 / 4), 20, 50, Color.WHITE);
		}
		polygons = array;
		return this;
	}
	public LineXGenerator2 generateGagdet(int num, int innerNum){
		FXPolygon[] array = new FXPolygon[num];
		disX = new float[num];
		for(int i = 0; i < num; i++) {
